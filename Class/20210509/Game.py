...
def fighting(status: list):
    s = 0.5  # attack speed
    monster_life = random.randint(200, 30000)
    print("Monster Life = %d" % monster_life)

    while True:
        while True:

            kill = input('Forbidden art activate? y/n: ')
            if (kill == 'y' and status[5] >= 1):
                attack = monster_life
                status[5] -= 1
                break
...
def store(status: list):
    while True:
        ...
        act = input("What do you want to buy 'l'/life, 'm'/magic, 'w'/weapon, 'k'/kill, 'q'/quit:")

        if (act == 'q' and status[2] >= 100):
            print("88")
            break
        elif (act == 'l' and status[2] >= 100):
            status[1] += random.randint(1, 3)
            status[2] -= 100
            print("Life=%d, Money=%d" % (status[1], status[2]))
        elif (act == 'm' and status[2] >= 100):
            status[3] += random.randint(1, 3)
            status[2] -= 100
            print("Magic=%d, Money=%d" % (status[3], status[2]))
        elif (act == 'w' and status[2] >= 100):
            status[4] += random.randint(1, 3)
            status[2] -= 100
            print('Purchase completed.')
        elif (act == 'k' and status[2] >= 500):
            status[5] += 1
            status[2] -= 500
            print('Purchase completed.')
        else:
...
#main program
sts = [1, 10, 10000, 10, 0, 1]  #是否生存/HP/錢/MP/武器攻擊力/必殺卷軸數量
...
while True:
    ...
    if (rev == "c"):
        ...
        if (sts[0] == 0):
            print("Game Over")
            break
        print("[[life = %d, money = %d, MP = %d, reel = %d]]" % (sts[1], sts[2], sts[3], sts[5]))
    elif (rev == "q"):
        print("88")
        break
    else:
